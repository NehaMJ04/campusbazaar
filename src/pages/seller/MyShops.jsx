...
export default function MyShops() {
  const navigate = useNavigate();
  // ── CHANGE 1: added useSearchParams ──
  ...
  // ── CHANGE 2: auto-open modal if ?edit=<shopId> is in the URL ──
  useEffect(() => {
    const editId = searchParams.get("edit");
    if (editId && shops.length > 0) {
      const shopToEdit = shops.find(s => s.id === editId);
      if (shopToEdit) setModalShop(shopToEdit);
    }
  }, [shops, searchParams]);
