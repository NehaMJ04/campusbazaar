...
async function fetchSellerStatus(userId) {
  // first get the college_id of the logged in user
  ...
  // then fetch their application using college_id
  const { data } = await supabase
    .from("seller_requests")
    .select("status, submitted_at, shop_name")
    .eq("college_id", userData.college_id)
    .single();

  if (data) setSellerStatus(data);
}
...
async function fetchUser() {
  ...
  fetchSellerStatus(user.id);  // add this
}
...
{sellerStatus ? (
  <div className="seller-status-card">
    <div className="seller-status-left">
      ...
      <p className="seller-shop-name">🏪 {sellerStatus.shop_name}</p>
      
      <p className="seller-applied-date">
  Applied on {new Date(sellerStatus.submitted_at).toLocaleDateString("en-IN", {
    day: "numeric", month: "long", year: "numeric"
  })}
</p>

    </div>
    <div className={`seller-status-badge ${sellerStatus.status}`}>
  ...
  <div className="seller-status-card no-application">
    <div className="seller-status-left">
      <h4>Want to sell on CampusBazaar?</h4>
      <p>Apply to become a seller and start your own shop.</p>
    </div>
    <button
      className="apply-seller-btn"
      onClick={() => navigate("/buyer/become-seller")}
    >
      Apply Now
    </button>
